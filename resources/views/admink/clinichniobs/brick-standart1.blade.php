<table style="text-align:center;overflow: scroll;width: 100%" class="table table-sm" >
...
<tr> 
  ...
  <input type="hidden" name="id_seminarus" value="{{$directions->id}}">

<input class="smol_input" type="hidden" name="user_id"  value="{{$user_inf->user_id}}"> 

<!-- <td>{!! $user_inf->user_id !!}</td> -->
<td title="{{$user_inf->user_id}}" >{!! $user_inf->surname !!} {!! $user_inf->name !!}</td>
...
<td><input type="hidden" title="{{$user_inf->surname}}"  size="1px" type="text" name="nine" value="0"></td>
...
<td <?php if ($user_inf->suma1 < 8) {$t="red";} 
  else {$t="green";} ?>
...
@endforeach
<tr>
<td colspan="13" style="font-size: small;font-style:italic;color:green;text-align:left;">1 - Заповнення документів, які засвідчують тимчасову непрацездатність, докменти на ЛКК, МСЄК.<br>
2 - Рецензія на медичну карту померлого хворого (надається в надрукованому вигляді).<br>
3 - Виконання хірургічної антисептичної обробки рук.<br>
4 - Виконання антисептичної обробки операційного поля.<br>
5 - Виконання прошивання та зав’язування трьох вузлів.<br>
6 - Підготовка відеоендоскопічного комплексу до роботи.<br>
7 - Техніка маніпулювання основними хірургічними інструментами з переміщення дрібних предметів (норматив 20 горошин за 70 сек).<br>
8 - Техніка роботи з відеокамерою (контроль введення інструментів, утримання «горизонту», демонстрація об’єкту спостереження під різними кутами й утримання його у центрі монітору.<br>
</td>
    </tr>
</table>
