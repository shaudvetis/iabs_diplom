...
<div class="card-tools" >
 <a href="{{asset('kardioclinobs')}}">Клінічне обстеження хворого</a>
 <p> <a href="{{asset('admink.controlmodyl.model_kardio')}}">Контроль модуля</a></p>
 </div>
 </div>
...
<td colspan="4" > 
  <div class='topFixed'>
  <a href="<?= '/kardio/?b2=test' . '&user_id=' . $user_inf['user_id'] ?>">
...
<table style="width: 100%" border="0" cellpadding="0" cellspacing="70" class="layer">
 ...
 <!--    <th >K</th> -->
  ...
  <tr>
      <form role="form" method="post"  action="{{route('kardio.store')}}">
               {{ csrf_field() }}  
...
<!-- Форма с выводом учебных элементов -->
 ...
 <!-- Задает блок кард на котором держаться все єлементі  --> 
  ...
  @endif
    ...
    @else
     <tr> <td></td>
     <td  style="size: 200px" >{!! $seminars->tema !!}</td>
     </tr>
     @endif
