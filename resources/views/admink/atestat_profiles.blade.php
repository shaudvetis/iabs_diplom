...
@include('layouts.instruction.kerivnuk.atestat')
@section ('content')
...
<a class="btn btn-tool"  style="float: right;" href=" {{back()->getTargetUrl()}}">Назад</a>
 ...
 <div class="table-responsive">
     ...
     <form  method="post" action="{{asset('atestat_profile')}}">
               {{ csrf_field() }}
...
<select name="nac_grade[]" class="form-control">
    ...
    <option value="3" @if(isset($diplom['atestatprofile']->nac_grade)) @if($diplom['atestatprofile']->nac_grade == 3) selected @endif @endif>Задовільно/Satisfactory</option> 
    <option value="9" @if(isset($diplom['atestatprofile']->nac_grade)) @if($diplom['atestatprofile']->nac_grade == 9) selected @endif @endif>Зараховано/Passed</option>   
    <option value="4" @if(isset($diplom['atestatprofile']->nac_grade)) @if($diplom['atestatprofile']->nac_grade == 4) selected @endif @endif>Добре/Good</option> 
    <option value="5" @if(isset($diplom['atestatprofile']->nac_grade)) @if($diplom['atestatprofile']->nac_grade == 5) selected @endif @endif>Відмінно/Excellent</option>   
</select>
</td>
