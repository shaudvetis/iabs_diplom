...
<tr text-align="left">
  ...
  <th style="height: 10px">Семінар 1</th>
  <th style="height: 10px">Семнар2</th>
  <th style="height: 10px">Семнар3</th>
 </tr>
   @foreach($suma as $user_inf)
<tr>
<td>{!! $user_inf['surname'] !!} {!! $user_inf['name'] !!} </a> </td>
@if(isset($user_inf['0']))
<td ><center>{!! $user_inf['0'] !!}</td>
@else  <td>{!!$user_inf['0']  = '' !!}  </td>
@endif  
@if(isset($user_inf['1']))
<td>{!!$user_inf['1']!!}</td>
@else  <td>{!!$user_inf['1']  = '' !!}  </td>
@endif           
@if(isset($user_inf['2']))
<td>{!! $user_inf['2'] !!}</td>
