...
@include('admink.include.naprav_settings.course_select') 
 ...
 <a href="{{asset('allcherevna')}}">Клінічне обстеження хворого</a>
...
@endforeach
 ...
 <!--    <th >K</th> -->
  ...
  <tr>
      <form role="form" method="post"  action="{{route('cherevna.store')}}">
               {{ csrf_field() }}  
...
@endisset
 ...
 <div  style='float:left; /*margin-top:280px;*/'>
  ...
  <td><button type="button" class="btn-sm btn-info"> {!! $seminars->npp !!}</button></td>
 <!--  Если нет вопросов не выводить кнопку с ? -->
 @if (empty($seminars->pract_nav))
    <td >{!! $seminars->tema !!}</td>
 @else
  <td ><button type="button" class="btn-sm btn-primary"  data-id="{{ $seminars->id }}"   data-toggle="modal" data-target="#exampleModal{{ $seminars->id }}" data-whatever="@mdo">?</button>{!! $seminars->tema !!}</td>
  @endif
        </tr>
     @else
        <tr> <td></td>
